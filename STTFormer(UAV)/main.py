...
class Processor():
    ...
    def check_isnan(self, data):
        for i in range(data.shape[0]):
            data_numpy = np.array(data[i])
            valid_frame_num = np.sum(data_numpy.sum(0).sum(-1).sum(-1) != 0)
            # print("data.shape[0]:{} {} {}".format(data.shape[0], i, valid_frame_num))
            if valid_frame_num == 0:
                print("valid_frame_num == 0")
                return True, i,  np.random.rand(155)
        return False, 0, np.random.rand(155)
        ...
        for ln in loader_name:
            ...
            for batch, (data, label, sampie) in enumerate(tqdm(self.data_loader[ln], desc="Evaluating", ncols=40)):
                ...
                with torch.no_grad():
                    ...
                    # print("shape:{}".format(output.data.cpu().numpy().shape))
                    _, predict_label = torch.max(output.data, 1)
                    pred_list.append(predict_label.data.cpu().numpy())
                    all_confidences.extend(output.detach().cpu().numpy())  # 收集预测并转换为 numpy
            ...
            if save_score:
                ...
                np.save(os.path.splitext(self.arg.weights)[0] + '.npy', np.array(all_confidences))
